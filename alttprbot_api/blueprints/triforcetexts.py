...
async def triforcetexts_sgl23():
    ...
    if not triforce_texts_config:
        return await render_template('error.html', logged_in=logged_in, user=user, title="Invalid key", message="This pool does not have a valid key set.")

    if key != triforce_texts_config.value:
        return await render_template('error.html', logged_in=logged_in, user=user, title="Invalid key", message="The key supplied is not valid.")

    session['sgl23_triforce_text_authorized'] = True

    return await render_template('triforce_text.html', logged_in=logged_in, user=user, pool_name=pool_name, author_name_field=True)
...
async def submit_sgl23():
    ...
    if not session.get('sgl23_triforce_text_authorized', False):
        return await render_template('error.html', logged_in=logged_in, user=user, title="Invalid key", message="The key in your session store is not correct.  This should not have happened.")

    payload = await request.form

    regex = re.compile(r"^[A-Za-z0-9 ?!,-….~～''↑↓→←あいうえおやゆよかきくけこわをんさしすせそがぎぐたちつてとげござなにぬねのじずぜはひふへほぞだぢまみむめもづでどらりるれろばびぶべぼぱぴぷぺぽゃゅょっぁぃぅぇぉアイウエオヤユヨカキクケコワヲンサシスセソガギグタチツテトゲゴザナニヌネノジズゼハヒフヘホゾダマミムメモヅデドラリルレロバビブベボパピプペポャュョッァィゥェォ]{0,19}$")
    ...
    await models.TriforceTexts.create(pool_name=pool_name, text=text, discord_user_id=None, author=payload['author_name'])
