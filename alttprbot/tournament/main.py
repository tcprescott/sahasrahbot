...
class SettingsSheet():
    ...
    async def _init(self):
        ...
        raise SettingsSubmissionNotFoundException(
            'Settings submission not found at <https://docs.google.com/spreadsheets/d/1GHBnuxdLgBcx4llvHepQjwd8Q1ASbQ_4J8ubblyG-0c/edit#gid=941774009>.  Please submit settings at <http://bit.ly/2Dbr9Kr>')
    ...
    def sanity_check(self):
        count = 0
        if not self.row['Dungeon Item Shuffle'] == 'Standard':
            count += 1
        if not self.row['Goal'] == 'Defeat Ganon':
            count += 1
        if not self.row['GT/Ganon Crystals'] == '7/7':
            count += 1
        if not self.row['World State'] == 'Open':
            count += 1
        if not self.row['Swords'] == 'Randomized':
            count += 1

        if self.row['Game Number'] == 1:
            return count == 0
        elif self.row['Game Number'] == 2:
            return count <= 1
        elif self.row['Game Number'] == 3:
            return count <= 2
        else:
            return False
