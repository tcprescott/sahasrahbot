...
async def update_tournament_race_status(room_name: int, status="STARTED"):
    await orm.execute(
        'UPDATE sgl2020_tournament_bo3 SET status = %s WHERE room_name = %s', [
            status, room_name]
    )
...
async def get_active_tournament_race(room_name: str):
    results = await orm.select(
        'SELECT * from sgl2020_tournament_bo3 where room_name=%s and status IS NULL;',
        [room_name]
    )
...
async def get_tournament_race_by_episodeid(episode_id: str):
    results = await orm.select(
        'SELECT * from sgl2020_tournament_bo3 where episode_id=%s;',
        [episode_id]
    )
    return results[0] if results else None
...
async def get_unrecorded_races():
    results = await orm.select(
        'SELECT * from sgl2020_tournament_bo3 where (`status` is null or `status` = "STARTED") and platform = "racetime";'
    )
    return results
...
async def delete_active_tournament_race(room_name: str):
    await orm.execute(
        'DELETE FROM sgl2020_tournament_bo3 WHERE room_name=%s and status IS NULL;',
        [room_name]
    )
