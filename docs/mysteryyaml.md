---
layout: default
title:  Mystery YAML file documentation
---
* Table of contents
{: toc}
...
This section lets you customize the changes that Link will be pre-equipped with specific items.
This is most commonly used to randomize a boots start, however it can be used to randomize whatever starting equipment you want.
...
This is probably the most complex section.  For a full list of available custom options, you may need to search ALTTPR's source code.
This is all undocumented and may work in unexpected ways.  Below are some settings that can be set, but it is not exhaustive.
...
If `countdown-ohko` is rolled, the `timed-ohko` section activates.
This allows you to specify the starting time on the OHKO (One Hit Knockout) clock, the amount of time the clocks grant, and the number of them added to th e pool.
A random value is chosen between the two numbers specified.  If this section is absent, the game will be traditional OHKO mode without any clocks.
...
These settings allow you to customize the behavior of the Triforce Hunt goal.
If chosen, it lets you specify the random range of the number of pieces required to meet the goal, the number of pieces in the pool, and the minimum difference between the number of goal pieces and the number of pool pieces.
...
If you didn't have enough already to take in, there is also a feature called "subweights".
Subweights allow you to allow you to roll a specific set of weights first, then merge those weights into the "parent" weights.
...
The [pogchampion_season5](https://github.com/tcprescott/sahasrahbot/blob/master/weights/pogchampion_season5.yaml) weightset has a good example of these in action.

# Preset rolling

You can have it choose a SahasrahBot preset.  This is rolled first before anything else.
If something other than `none` is selected, it'll create a game using that preset instead.
