...
class Tournament(commands.Cog):
    ...
    async def create_races(self):
        ...
        for tournament in tournaments.TOURNAMENT_DATA:
            try:
                episodes = await speedgaming.get_upcoming_episodes_by_event(tournament.event_slug, hours_past=0.5, hours_future=.75)
            ...
            for episode in episodes:
                ...
                try:
                    await tournaments.create_tournament_race_room(tournament.event_slug, episode['id'])
                except Exception as e:
                    logging.exception(
    ...
    async def find_unsubmitted_races(self):
        ...
        for tournament in tournaments.TOURNAMENT_DATA:
            if not tournament.submission_form:
                continue

            try:
                episodes = await speedgaming.get_upcoming_episodes_by_event(tournament.event_slug, hours_past=0, hours_future=168)
            except Exception as e:
            ...
            for episode in episodes:
                logging.info(episode['id'])
                try:
                    tournament_race = await tournaments.fetch_tournament_handler(tournament.event_slug, episode['id'])
                    ...
                    if tournament.audit_channel:
                        await tournament.audit_channel.send(
    ...
    async def scheduling_needs(self):
        logging.info("scanning SG schedule for tournament races to create")
        for tournament in tournaments.TOURNAMENT_DATA:
            if tournament.scheduling_needs_channel is None:
                continue

            try:
                episodes = await speedgaming.get_upcoming_episodes_by_event(tournament.event_slug, hours_past=0, hours_future=48)
            except Exception as e:
                logging.exception("Encountered a problem when attempting to retrieve SG schedule.")
                continue
            ...
            for episode in episodes:
                broadcast_channels = [c['slug'] for c in episode['channels'] if c['id'] not in [0, 31, 36, 62, 63, 64, 65] and c['language'] == tournament.lang]
                ...
                commentators_approved = [p for p in episode['commentators'] if p['approved'] and p['language'] == tournament.lang]
                ...
                trackers_approved = [p for p in episode['trackers'] if p['approved'] and p['language'] == tournament.lang]
                ...
                if broadcast_channels[0] in ['ALTTPRandomizer', 'ALTTPRandomizer2', 'ALTTPRandomizer3', 'ALTTPRandomizer4', 'ALTTPRandomizer5', 'ALTTPRandomizer6']:
                    broadcasters_approved = [p for p in episode['broadcasters'] if p['approved'] and p['language'] == tournament.lang]
            ...
            try:
                bot_message = False
                async for message in tournament.scheduling_needs_channel.history(limit=50):
                ...
                if not bot_message:
                    await tournament.scheduling_needs_channel.send(embed=embed)
            except Exception:
