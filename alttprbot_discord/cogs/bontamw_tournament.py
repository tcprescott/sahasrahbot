...
class BontaMultiworldTournament(commands.Cog):
    ...
    @commands.command()
    async def mwresume30(self, ctx, token, port):
    ...
    @commands.command()
    @commands.is_owner()
    async def mwmsg30(self, ctx, token, msg):
        await cleanup_procs()
        global MULTIWORLDS
...
def setup(bot):
    bot.add_cog(BontaMultiworldTournament(bot))
